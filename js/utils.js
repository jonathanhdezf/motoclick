...
(function initScrollProgress() {
  const setupBar = () => {
    ...
    // Inject styles inline to avoid main.css dependency
    const style = document.createElement('style');
    style.textContent = `
      .scroll-progress-container {
        position: fixed; top: 0; right: 0; width: 4px; height: 100vh;
        background: rgba(255, 255, 255, 0.03); z-index: 10005; pointer-events: none;
      }
      .scroll-progress-bar {
        width: 100%; height: 0%; background: linear-gradient(135deg, #00B347, #00E65B);
        box-shadow: 0 0 15px rgba(0, 230, 91, 0.4); transition: height 0.1s linear;
        border-bottom-left-radius: 4px; border-bottom-right-radius: 4px;
      }
      @media (max-width: 768px) { .scroll-progress-container { width: 3px; } }
