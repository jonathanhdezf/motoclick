...
/* ── Global Scroll Progress Injector ── */
(function initGSProgress() {
  const setupGSBar = function() {
    if (document.getElementById('gsScrollProgress')) return;

    var style = document.createElement('style');
    style.innerHTML = '.gs-progress-container { position: fixed; top: 0; right: 0; width: 4px; height: 100vh; background: rgba(255, 255, 255, 0.03); z-index: 10005; pointer-events: none; }' + 
      '.gs-progress-bar { width: 100%; height: 0%; background: linear-gradient(135deg, #00B347, #00E65B); box-shadow: 0 0 15px rgba(0, 230, 91, 0.4); transition: height 0.1s linear; border-bottom-left-radius: 4px; border-bottom-right-radius: 4px; }' +
      '@media (max-width: 768px) { .gs-progress-container { width: 3px; } }' +
      '.gs-progress-container.driver-mode .gs-progress-bar { background: linear-gradient(135deg, #3b82f6, #2563eb); box-shadow: 0 0 15px rgba(59, 130, 246, 0.5); }';
    document.head.appendChild(style);

    var container = document.createElement('div');
    container.className = 'gs-progress-container';
    if (window.location.pathname.indexOf('/repartidor/') !== -1) { container.classList.add('driver-mode'); }

    var bar = document.createElement('div');
    bar.className = 'gs-progress-bar';
    bar.id = 'gsScrollProgress';
  ...
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', setupGSBar);
  } else {
    setupGSBar();
  }
})();
